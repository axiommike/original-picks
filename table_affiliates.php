...
function generateTable($pdo, $league, $title)
{
    ...
    $stmt = $pdo->prepare('SELECT * FROM affiliation_history WHERE league = :league ORDER BY nhlTeam ASC, startDate ASC');
    ...
    foreach ($teams as $index => $row) {
        ...
        // Text colours
        $statusColor = $row['teamStatus'] === 'Inacive' ? '#ff0000' : $textColor;
